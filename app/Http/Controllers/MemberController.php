...
class MemberController extends Controller
{
    ...
    public function getIndex()
    {
        ...
        return view('member.list', compact(['userList', 'amountPerPage', 'totalCount']));
    }
    ...
    public function postLogin(Request $request)
    {
        ...
        $user->update([
            ...
            'lastlogin_at' => Carbon::now()->toDateTimeString(),
        ]);
        //移除重新設定密碼的驗證碼
    ...
    public function getRegister()
    {
        ...
        return view('member.register', compact('allowedEmailsArray'));
    }
    ...
    {
        $user = User::where('confirm_code', '=', $token)->whereNull('confirm_at')->first();
        if (!$user) {
    ...
    public function getResetPassword($token = null)
    {
        if (DB::table('password_resets')->where('token', '=', $token)->count()) {
            ...
            //檢查使用者是否存在
            if ($user) {
                return view('member.reset-password', compact(['user', 'token']));
            }
        }
    ...
    public function getProfile($uid = null)
    {
        ...
        return view('member.other-profile', compact(['user', 'showUser']));
    }
    ...
    public function postEditProfile(Request $request)
    {
        ...
        /* @var User $user */
        $user = auth()->user();
        $user->update($request->only('nickname'));
